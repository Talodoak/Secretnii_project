  ...
  private socketHandler():void {
    ...
    this.io.on('connection', (socket:Socket):void => {
      ...
      userSet.forEach((val:IUser) => {
        if (val.name == socket.request.session.name || val.uniqueID == socket.request.session.id) {
      ...
      let user:IUser = { uniqueID: socket.request.session.uid, name: socket.request.session.username };
      ...
      userSet.add(user);
      var serverMsg:string = `<p class="message"><b>Server says:</b><br>
